...
- [x] Добавлены `tariff.code`: `transit_month_pack`, `compatibility_deep_dive`, `return_pack`.
- [x] Введена связка add-on с базовым заказом (`parent_order_id` в `orders`, без изменения жизненного цикла базового заказа).
- [x] Добавлены API ` /api/v1/addons/*`.
- [x] Добавлены Celery задачи генерации add-on отчетов.
- [~] Добавлены точки офферинга: основной канал — in-app в кабинете (post-report/dashboards); вторичный догоняющий канал — email/push запланирован следующим инкрементом.
- [x] Добавлен глобальный switch в админке: включить/выключить add-on контур целиком (kill switch для продаж и офферинга) — размещается в `Feature Flags` (`/admin/flags`, ключ `addons_enabled`).
- [x] Настроены KPI и события аналитики для add-on с первого релиза; A/B тесты отложены, но контракт событий готов к включению.

**Ожидаемый эффект:** рост AOV, повторной выручки и LTV.

**Примечание по текущему статусу:** в текущем релизе запущен backend/API/analytics + in-app офферинг с kill switch `addons_enabled`; email/push догоняющий канал вынесен в следующий инкремент без изменения контрактов `4.3`.

**Доп. разделы натального отчёта (report / bundle):** для тарифов `report` и `bundle` пользователь может включить до четырёх платных тематических блоков; их цены и процент multi-скидки при выборе двух и более блоков задаются в `app_settings` через админку. Итог заказа — это база тарифа после валидного маркетингового промокода (если он передан) **плюс** отдельная строка по тумблерам; промокод **не** распространяется на доп. разделы. Выбор фиксируется в `orders.report_option_flags`, попадает в промпт генерации отчёта и в дополнение к ключу LLM-кэша; отображение и приём опций в API зависят от флага `report_upsell_sections_enabled`.
...
- [x] Решение по `app/tasks/scheduler.py`: удалить (файл удалён).
- [ ] Решение по `app/utils/landing_html.py` + связанным тестам.
- [x] Решение по `HTML макеты/`: перенесены в `docs/archive/HTML макеты`.
- [ ] Решение по legacy static `frontend/public/static/*`.
- [x] Решение по roadmap-спекам: перенесены из `app/` в `docs/roadmap/`.
...
- `ADMIN_ANALYTICS_AND_METRICS.md` — указатель и источники истины; полная матрица §2–5 и закрытый чеклист — `docs/archive/2026-04-16/ADMIN_ANALYTICS_AND_METRICS.v1-reference.archived-2026-04-16.md`
- `PRODUCT_FLOW_AND_BUSINESS_LOGIC.md`
- `PRODUCTION_READINESS_AND_GROWTH_PLAN.md`
- `PRODUCT_AND_ECONOMICS_STRATEGY_3_5_YEARS.md`
- `KERYKEION_V5_MIGRATION_PLAN.md`
