...
Корень сайта и пользовательские маршруты работают как **React SPA**. В dev используйте `http://localhost:5173`; API идёт через прокси Vite (`/api/v1` → FastAPI).
...
Это выполнит сборку и скопирует `dist` в `../frontend-dist`. Caddy проксирует `/api/*` на FastAPI и отдаёт SPA fallback (`try_files ... /index.html`) для пользовательских маршрутов.
...
**Токены:** React SPA сохраняет токен в **`localStorage`** через persist Zustand (`astrogen_auth_token`). Логика миграции `sessionStorage` сохранена в `main.tsx` для совместимости со старыми сессиями.

**Telegram Mini App:** в BotFather укажите **Web App URL** `https://ВАШ-ДОМЕН/` (корень SPA).
